...
util_include_path = os.path.join(current_dir, "../../util")
sources = [os.path.join(current_dir, filename) for filename in ["block_mma16816_2_4_1-2.cu"]]
...
for exp in exps:
    ...
    a = torch.arange(M*K, device="cuda", dtype=torch.float32).view(M,K)
    a = a % 64
    a = a.bfloat16()'''
    a = torch.arange(M*K, device="cuda", dtype=torch.float32).view(M,K)
    a = a.bfloat16()
    #print(a)
    b = (torch.arange(N*K, device="cuda", dtype=torch.float32).reshape(K,N)).bfloat16()
    c = torch.randn(M, N, device="cuda", dtype=torch.float32)
    #print(b,b.shape,b.t().shape)

    # Case 1: MM
    kernel_output = lib.block_mma_bf16_bf16_bf16_fp32(a, b.t().contiguous(), None)
    #print(kernel_output,kernel_output.shape)
    if not ENABLE_PROF:
        torch_output = torch.matmul(a.float(), b.float()).bfloat16()
